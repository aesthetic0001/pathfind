...
class AStar {
  ...
  compute () {
    ...
    while (!this.openHeap.isEmpty()) {
      ...
      const currentXDirection = node.parent ? node.data.x - node.parent.data.x : 0

      // console.log(`Current X Direction: ${currentXDirection}`)

      const neighbors = this.movements.getNeighbors(node.data, currentXDirection)
