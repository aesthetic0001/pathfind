...
function inject (bot) {
  ...
  function monitorMovement () {
    ...
    if (Math.abs(dx) <= (!path[1] ? 0.25 : (yChangeFromGround >= 1.5 ? Math.max(2, yChangeFromGround / 2.5) : 0.5)) && Math.abs(dz) <= (!path[1] ? 0.25 : (yChangeFromGround > 1.5 ? Math.max(2, yChangeFromGround / 2.5) : 0.5)) && Math.abs(dy) < 12) {
